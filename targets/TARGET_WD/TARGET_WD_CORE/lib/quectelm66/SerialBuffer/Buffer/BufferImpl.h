...
public:
     ...
     */
	BufferImpl(uint32_t size = 0x100);
...
template <class T>
	inline void BufferImpl<T>::put(T data)
{
    _buf[_wloc++] = data;
    _wloc %= (_size-1);
...
template <class T>
	inline T BufferImpl<T>::get(void)
{
...
template <class T>
	inline T *BufferImpl<T>::head(void)
{
    T *data_pos = &_buf[0];
...
template <class T>
	inline uint32_t BufferImpl<T>::available(void) {
